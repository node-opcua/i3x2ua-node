...
async function main(): Promise<void> {
  ...
  logger.info('i3x2ua-node starting...');
  ...
  if (!config.skipOpcuaConnect) {
    ...
    logger.info('Skipping OPC UA connection (I3X_SKIP_OPCUA_CONNECT=1)');
  }

  // 5. Start HTTP server
  await app.listen({ port: config.port, host: config.host });
  logger.info(`i3x2ua-node listening on http://${config.host}:${config.port}`);
